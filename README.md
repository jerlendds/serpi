# SERPI - Search engine result page interface
...
In that case, you will need to use a fake local domain (`dev.serpi.ca`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.serpi.ca` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.
...
- Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.serpi.ca`.

The new line might look like:

```
192.168.99.100    dev.serpi.ca
```

- Save the file.
  - **Note for Windows**: Make sure you save the file as "All files", without an extension of `.txt`. By default, Windows tries to add the extension. Make sure the file is saved as is, without extension.

...that will make your computer think that the fake local domain is served by that custom IP, and when you open that URL in your browser, it will talk directly to your locally running server when it is asked to go to `dev.serpi.ca` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.serpi.ca`.

After performing those steps you should be able to open: http://dev.serpi.ca and it will be server by your stack in `localhost`.

Check all the corresponding available URLs in the section at the end.

### Change the development "domain"

If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.serpi.ca`).
...
```bash
STACK_NAME=stage-serpi-ca sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.serpi-ca.app-db-data == true
```
...
```bash
docker node update --label-add serpi-ca.app-db-data=true dog.example.com
```

- Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stage-serpi-ca.app-db-data=true cat.example.com
```
...
Frontend: https://serpi.ca

Backend: https://serpi.ca/api/

Automatic Interactive Docs (Swagger UI): https://serpi.ca/docs

Automatic Alternative Docs (ReDoc): https://serpi.ca/redoc

PGAdmin: https://pgadmin.serpi.ca

Flower: https://flower.serpi.ca

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://stage.serpi.ca

Backend: https://stage.serpi.ca/api/

Automatic Interactive Docs (Swagger UI): https://stage.serpi.ca/docs

Automatic Alternative Docs (ReDoc): https://stage.serpi.ca/redoc

PGAdmin: https://pgadmin.stage.serpi.ca

Flower: https://flower.stage.serpi.ca
...
Frontend: http://dev.serpi.ca

Backend: http://dev.serpi.ca/api/

Automatic Interactive Docs (Swagger UI): https://dev.serpi.ca/docs

Automatic Alternative Docs (ReDoc): https://dev.serpi.ca/redoc

PGAdmin: http://dev.serpi.ca:5050

Flower: http://dev.serpi.ca:5555

Traefik UI: http://dev.serpi.ca:8090
